...
const AREA_CONFIG = {
  "Reparo Mecânico": {
    titulo: "Mecânico",
    lista: REPAROS_MECANICOS_LAVADORAS,
  },
  "Reparo Elétrico": {
    titulo: "Elétrico",
    lista: REPAROS_ELETRICOS_LAVADORAS,
  },
  "Reparo Estético": {
    titulo: "Estético",
    lista: REPAROS_ESTETICOS_LAVADORAS,
  },
};
...
export default function ReparosLavadorasV2Page() {
  ...
  const configArea = areaAtual
    ? AREA_CONFIG[areaAtual]
    : null;
  ...
  function toggleServico(item) {
    setExecucao((current) => {
      const existe =
        current.servicos.includes(item);

      return {
        ...current,
        servicos: existe
          ? current.servicos.filter(
              (currentItem) =>
                currentItem !== item
            )
          : [
  ...
  async function salvar() {
    if (!canSave) {
      setMensagem(
        "Informe o diagnóstico e selecione ao menos um serviço executado."
      );
      return;
    }
    ...
    try {
      ...
      const haviaMaisAreas =
        areasPendentes.length > 1;
      ...
      setMensagem(
        haviaMaisAreas
          ? `Reparo da OS ${numeroOs} concluído nesta área. A OS permanece em reparo para a próxima especialidade.`
    ...
    <div className="mx-auto max-w-[1500px]">
      <div className="border-b border-slate-200 pb-7">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
          Execute o reparo por especialidade e registre serviços, peças e diagnóstico técnico.
        </p>
      </div>
      ...
      <section className="mt-8">
        ...
        >
          <div className="mb-4 text-sm font-black text-slate-800">
            {configArea
              ? `Reparo ${configArea.titulo}`
              : "Selecione uma especialidade"}
          </div>

          <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
            {(configArea?.lista || []).map(
              (item) => {
                const ativo =
                  execucao.servicos.includes(
                    item
                  );
