...
const AREAS_REPARO = [
  {
    label: "Reparo Mecânico",
    key: "mecanico",
  },
  {
    label: "Reparo Elétrico",
    key: "eletrico",
  },
  {
    label: "Reparo Estético",
    key: "estetico",
  },
];
...
export default function TriagemLavadorasV2Page() {
  ...
  const canSave = useMemo(() => {
    ...
    if (
      triagem.precisa_reparo === null
    ) {
      return false;
    }

    if (
      triagem.verificacoes.length === 0
    ) {
      return false;
    }
  ...
  function toggleVerificacao(item) {
    setTriagem((current) => {
      ...
      return {
        ...current,
        verificacoes: existe
          ? current.verificacoes.filter(
              (currentItem) =>
                currentItem !== item
            )
          : [
              ...current.verificacoes,
              item,
            ],
      };
    });
  ...
  async function salvar() {
    if (!canSave) {
      setMensagem(
        "Conclua as verificações e o direcionamento antes de salvar a triagem."
      );
    ...
    try {
      ...
      setMensagem(
        `OS ${numeroOs} triada com sucesso.`
      );
    } catch (error) {
      setMensagem(
    ...
    <div className="mx-auto max-w-[1500px]">
      ...
      <div className="border-b border-slate-200 pb-7">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
          Identifique a OS, execute as verificações técnicas e defina o direcionamento do equipamento.
        </p>
      </div>
  ...
  >
    ...
    {osFiltradas.map((os) => (
      ...
      <section className="mt-8">
        ...
        >
          {REPAROS_LAVADORAS.map(
            (item) => {
              const ativo =
                triagem.verificacoes.includes(
                  item
                );
      ...
      <section className="mt-8">
        ...
        <textarea
          ...
          disabled={!selectedOs}
          rows={4}
          placeholder="Registre evidências, anomalias encontradas e observações adicionais."
          className="
