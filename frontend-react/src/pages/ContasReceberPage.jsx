...
export default function ContasReceberPage() {
  ...
  async function handleSync() {
    ...
    try {
      ...
      setStatus(`Sincronizado! ${totalCR} contas a receber atualizadas.`);
      await loadData();
    } catch (err) {
  ...
  const totalRecebido = useMemo(() =>
    rows.filter((r) => r.situacao === "recebido" || r.situacao === "pago").reduce((s, r) => s + (r.valor || 0), 0),
    [rows]
  );
    ...
    <div className="space-y-6">
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-3">
        <div className="rounded-[24px] bg-green-50 p-5">
          <div className="text-xs font-semibold uppercase tracking-widest text-slate-500 mb-3">Total a Receber</div>
          <div className="text-2xl font-black text-green-700">{fmt(totalAberto)}</div>
        </div>
        <div className="rounded-[24px] bg-purple-50 p-5">
          <div className="text-xs font-semibold uppercase tracking-widest text-slate-500 mb-3">Total Recebido</div>
          <div className="text-2xl font-black text-[#6B1F87]">{fmt(totalRecebido)}</div>
        </div>
        ...
        <div className="flex flex-wrap gap-3 items-end justify-between">
          <div className="flex flex-wrap gap-3 items-end">
            <div>
              ...
              <input type="text" value={filters.busca} onChange={(e) => update("busca", e.target.value)} className={inputClass()} placeholder="Cliente..." />
            </div>
            <div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#E9D5FF]">
                <th className="text-left py-3 px-4 text-xs font-bold uppercase tracking-wide text-slate-400">Cliente</th>
                ...
                rows.map((row) => {
                  ...
                  return (
                    <tr key={row.id} className={`border-b border-[#F4ECFA] hover:bg-[#FCFAFF] transition ${vencido ? "bg-red-50/50" : ""}`}>
                      ...
                      <td className="py-3 px-4 text-right font-bold text-green-700">{fmt(row.saldo)}</td>
            ...
            {rows.length > 0 && (
              <tfoot>
                <tr className="bg-[#4C1D95] text-white">
                  ...
                  <td className="py-3 px-4 text-right font-bold text-green-300">{fmt(rows.reduce((s, r) => s + (r.saldo || 0), 0))}</td>
                  <td></td>
                </tr>
              </tfoot>
            )}
